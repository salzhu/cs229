...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # print("key shape", key.shape) ## [bs = 2, num_attention_heads = 12, seq_len = 8, attention_head_size = 64].
    Kt = key.permute(0, 1, 3, 2)
    QKt = torch.matmul(query, Kt) 
    # print("Qkt shape" ,QKt.shape) ## ## [bs = 2, num_attention_heads = 12, seq_len = 8, seq_len = 8].

    QKt = QKt / self.attention_head_size ** 0.5
    QKtmasked = QKt + attention_mask
    softmaxQKt = F.softmax(QKtmasked, dim = 3) ##check dim
 
    attention = torch.matmul(softmaxQKt, value)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    outputTransofrmed = dense_layer(output)
    outputDropout = dropout(outputTransofrmed)
    return ln_layer(input + outputDropout)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    ### TODO
    multiHeadOutput = self.self_attention(hidden_states, attention_mask)
    multiHeadOutput = self.add_norm(hidden_states, multiHeadOutput, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    feedForwardInput = multiHeadOutput
    feedForwardOutput = self.interm_af(self.interm_dense(feedForwardInput))
    finalOutput = self.add_norm(feedForwardInput, feedForwardOutput, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    # raise NotImplementedError
    added_embeds = inputs_embeds + pos_embeds + tk_type_embeds
    final = self.embed_dropout(self.embed_layer_norm(added_embeds))
    return final
