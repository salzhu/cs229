...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            ...
            logits = model.predict_paraphrase2(b_ids1, b_mask1, b_ids2, b_mask2)
        ...
        for step, batch in enumerate(tqdm(sts_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            ...
            logits = model.predict_similarity2(b_ids1, b_mask1, b_ids2, b_mask2)
...
def model_eval_test_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            ...
            logits = model.predict_paraphrase3(b_ids1, b_mask1, b_ids2, b_mask2)
            y_hat = logits.sigmoid().round().flatten().cpu().numpy()
        ...
        for step, batch in enumerate(tqdm(sts_dataloader, desc=f'eval', disable=TQDM_DISABLE)):
            ...
            logits = model.predict_similarity3(b_ids1, b_mask1, b_ids2, b_mask2)
            y_hat = logits.flatten().cpu().numpy()
